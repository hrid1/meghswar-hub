...
const CreateRiderForm = () => {
  ...
  const onSubmit = async (data: RiderFormData) => {
    ...
    try {
      ...
      // Updated payload structure to match the required format
      const payload = {
        full_name: data.name,
        phone: data.mobile.replace(/[^0-9]/g, ''), // Remove non-numeric characters
        email: data.email,
        password: data.password,
      ...
      if (!riderResponse.ok) {
        ...
        throw new Error(errorData?.message || 'Failed to create rider');
      }

      const responseData = await riderResponse.json();
