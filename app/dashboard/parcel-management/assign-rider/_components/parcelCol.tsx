...
export const parcelColumns = (onClickUpdate: any) => [
  {
    ...
    render: (row: any) => (
      <div className="">
        ...
        <span className="text-xs text-gray-500 block">MID: {row.merchant_order_id}</span>
      </div>
    ),
  },
  {
    ...
    render: (row: any) => (
      <div className="flex flex-col">
        ...
        {row.customer_secondary_phone && (
          <span className="text-xs text-gray-400">Alt: {row.customer_secondary_phone}</span>
        )}
  ...
  },
  {
    ...
    render: (row: any) => (
      <div className="text-sm text-gray-600">
        <div>{row.delivery_area?.area}</div>
        <div className="text-xs text-gray-400">{row.delivery_area?.zone}</div>
        <div className="text-xs text-gray-400">{row.delivery_area?.city}, {row.delivery_area?.division}</div>
      </div>
    ),
  },
  {
  ...
  
  {
    ...
    render: (row: any) => (
      <div>
        ...
        {row.is_cod ? (
          <span className="text-xs bg-green-100 text-green-700 px-1 py-0.5 rounded ml-1">COD</span>
        ) : (
          <span className="text-xs bg-gray-100 text-gray-700 px-1 py-0.5 rounded ml-1">Prepaid</span>
        )}
